...
def solution_two(A):
    ...
    for sp, item in enumerate(A, start=1):
        if sp == len(A):
            return min
        counter += item
        right = total_sum - counter
        result = abs(counter - right)
        if result < min:
            min = result
...
class Tests(unittest.TestCase):
    ...
    def test_two_elements(self):
        A = [1, 2]
        self.assertEqual(solution_two(A), 1)
        A = [10, 200]
        self.assertEqual(solution_two(A), 190)
        A = [-1000, 1000]
        s = solution_two(A)
        self.assertEqual(s, 2000)
